    ...
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule"
        ...
        <el-form-item label="分类id" prop="categoryId">
          <el-select v-model="searchInfo.categoryId" :clearable="true">
        ...
        <el-form-item label="开始售卖时间" prop="sellBeginTime">
          ...
          <el-date-picker v-model="searchInfo.startSellBeginTime" type="datetime" placeholder="开始日期"
            :disabled-date="time => searchInfo.endSellBeginTime ? time.getTime() > searchInfo.endSellBeginTime.getTime() : false" />
          —
          <el-date-picker v-model="searchInfo.endSellBeginTime" type="datetime" placeholder="结束日期"
            :disabled-date="time => searchInfo.startSellBeginTime ? time.getTime() < searchInfo.startSellBeginTime.getTime() : false" />
        </el-form-item>
        <el-form-item label="结束售卖时间" prop="sellEndTime">
          ...
          <el-date-picker v-model="searchInfo.startSellEndTime" type="datetime" placeholder="开始日期"
            :disabled-date="time => searchInfo.endSellEndTime ? time.getTime() > searchInfo.endSellEndTime.getTime() : false"></el-date-picker>
          —
          <el-date-picker v-model="searchInfo.endSellEndTime" type="datetime" placeholder="结束日期"
            :disabled-date="time => searchInfo.startSellEndTime ? time.getTime() < searchInfo.startSellEndTime.getTime() : false"></el-date-picker>
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="id"
        ...
        <el-table-column align="left" label="图片" prop="pic" width="120">
          <template #default="scope">
            <CustomPic pic-type="file" style="margin-top:8px" :pic-src="scope.row.pic" />
          </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="分类" prop="categoryId" width="120">
          <template #default="scope">{{ formatByList(scope.row.categoryId, goodsCategoryList, 'id', 'name')
            }}</template>
        </el-table-column>
        ...
        <el-table-column align="left" label="运费模版" prop="logisticsId" width="120">
          <template #default="scope">{{ formatByList(scope.row.logisticsId, beeLogisticsList, 'id', 'name')
            }}</template>
        </el-table-column>
          ...
          <template #default="scope">
            <div>
              ...
              <el-button type="primary" link class="table-button"
                @click="gotoAdditionPage(scope.row)">查看配件信息</el-button>
            </div>
            <div>
            ...
            <div>
              <el-button type="primary" link icon="edit" class="table-button"
                @click="updateBeeShopGoodsFunc(scope.row)">变更</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
      ...
      <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="售后说明:" prop="afterSale">
          <bee-mul-select v-model="formData.afterSale" :options="beeGoodsAfterSale" :field="'value'"
            :label="'label'"></bee-mul-select>
        </el-form-item>
        <el-form-item label="分类:" prop="categoryId">
          <el-select v-model="formData.categoryId" :clearable="true">
            <el-option v-for="(item, key) in goodsCategoryList" :key="item.id" :value="item.id" :label="item.name" />
          </el-select>
        </el-form-item>
        <el-form-item label="会员卡专用商品:" prop="isMemberCardProd">
          <el-switch v-model="formData.isMemberCardProd" active-color="#13ce66" inactive-color="#ff4949" active-text="是"
        ...
        <el-form-item label="隐藏:" prop="hidden">
          <el-switch v-model="formData.hidden" active-color="#13ce66" inactive-color="#ff4949" active-text="是"
        ...
        <!--            </el-form-item>-->
        <el-form-item label="运费模版:" prop="logisticsId">
          <el-select v-model="formData.logisticsId" :clearable="true">
            <el-option v-for="(item, key) in beeLogisticsList" :key="item.id" :value="item.id" :label="item.name" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="商品评分:" prop="numberGoodReputation">
          <el-input-number v-model="formData.numberGoodReputation" style="width:100%" :precision="2"
            :clearable="true" />
        </el-form-item>
        ...
        <el-form-item label="图片:" prop="pic">
          ...
          <SelectImage v-model="formData.pic" />
        </el-form-item>
        ...
        <el-form-item label="商品属性:" prop="propertyIds">
          <bee-mul-select v-model="formData.propertyIds" :options="goodsPropList.filter(item => !item.propertyId)"
            :field="'id'" :label="'name'"></bee-mul-select>
        </el-form-item>
        ...
        <el-form-item label="商店id:" prop="shopId">
          <el-select v-model="formData.shopId" clearable placeholder="请选择" :clearable="false">
            <el-option v-for="(item, key) in shopList" :key="key" :label="item.id + (item.isDeleted ? '(已删除)' : '')"
              :value="parseInt(item.id)" />
          </el-select>
        </el-form-item>
        <el-form-item label="商品状态:" prop="status">
          <el-select v-model="formData.status" clearable placeholder="请选择" :clearable="false">
            <el-option v-for="(item, key) in beeGoodsStatus" :key="key" :label="item.label"
        ...
        <el-form-item label="开始售卖时间:" prop="sellBeginTime">
          <el-date-picker v-model="formData.sellBeginTime" type="date" style="width:100%" placeholder="选择日期"
            :clearable="true" />
        </el-form-item>
        <el-form-item label="结束售卖时间:" prop="sellEndTime">
          <el-date-picker v-model="formData.sellEndTime" type="date" style="width:100%" placeholder="选择日期"
            :clearable="true" />
        </el-form-item>
