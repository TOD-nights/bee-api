...
import { getAllBeeShopInfos } from "@/plugin/beeshop/api/beeShopInfo.js";
  ...
  if (table.code === 0) {
    tableData.value = table.data.list.map((item) => {
      return {
        ...item,
        shopInfoNames:
          item.shopInfos == null || item.shopInfos.length == 0
            ? "暂未设置"
            : item.shopInfos.map((info) => info.name).join(","),
      };
    });
...
const editAuthority = (row) => {
  ...
  for (const key in form.value) {
    ...
    form.value.shopInfos = row.shopInfos.map((item) => item.id);
  }
  setOptions();
  dialogFormVisible.value = true;
  if (shopInfos.value.length == 0) {
...
const getAllShopInfos = () => {
  return getAllBeeShopInfos().then((res) => {
    if (res.code == 0) {
      shopInfos.value = res.data.list;
    }
