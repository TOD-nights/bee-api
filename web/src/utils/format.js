...
export const buildTreeMap = (tree, m) => {
  if (tree.length > 0) {
    for (let i = 0; i < tree.length; i++) {
      m[tree[i].id] = tree[i]
      if (tree[i].childs && tree[i].childs.length > 0) {
        buildTreeMap(tree[i].childs, m)
      }
    }
  }
}
...
export const formatByList = (e, list, field, label) => {
  if (!e) return ''
...
export const findValByTree = (e, tree, field, label) => {
  for (let i = 0; i < tree.length; i++) {
    if (tree[i][field] == e) {
      return tree[i][label]
    }
    if (tree[i].childs && tree[i].childs.length > 0) {
      const res = findValByTree(e, tree[i].childs, field, label)
      if (res) {
        return res
      }
    }
...
export const formatStrByTree = (e, tree, field, label) => {
  const val = findValByTree(e, tree, field, label)
  return val || 'unknown'
}
